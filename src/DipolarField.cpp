...
std::vector<double> DipolarInteractions::DipolarInteractionClassic(std::vector<double> mxTerms, std::vector<double> myTerms,
    ...
    for (int i = 0; i < mxTerms.size(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            double DipoleValue = _simParams->dipoleConstant * ((3.0 * positionVector[j] * influencingSiteDotPosition) / positionVector_fifth - influencingSite[j] / positionVector_cubed);
...
std::vector<double> DipolarInteractions::DipolarInteractionInterlayerAdjacent(std::vector<std::vector<double>>& mTermsChain1,
    ...
    if (currentSite > _simParams->numSpinsDamped and currentSite <= chainTwoOffset) {
        ...
        for (int j = 0; j < 3; j++) {
            double DipoleValue = _simParams->dipoleConstant * ((3.0*positionVector[j]*influencingSiteDotPosition) / positionVector_fifth
...
std::vector<double> DipolarInteractions::DipolarInteractionIntralayerDebug(std::vector<std::vector<double>>& mTerms, int& numNeighbours,
    ...
    // Here to improve readability; could be removed to improve performance
    std::vector<double> originSite = {mxTerms[originIndex], myTerms[originIndex], mzTerms[originIndex]};
    if (_simFlags->debugFunc) { std::cout << " INTRA Origin: [" << originSite[0] << ", " << originSite[1] << ", " << originSite[2] << "] | ";}
    ...
    if (_simFlags->debugFunc) { std::cout << "\nDB2.5.0 (" << i+1 << ") | "; }
        ...
        // Moment at site i. Here to improve readability; could be removed to improve performance
        std::vector<double> influencingSite = {mxTerms[i], myTerms[i], mzTerms[i]};
        if (_simFlags->debugFunc) { std::cout << "INTRA Influe: [" << influencingSite[0] << ", " << influencingSite[1] << ", " << influencingSite[2] << "] | ";}
        if (influencingSite[0] == 0.0 && influencingSite[1] == 0.0 && influencingSite[2] == 0.0) {
            // If influencing site components are all zero, then they don't impact the calculation. So can be skipped
...
std::vector<double> DipolarInteractions::DipolarInteractionInterlayerDebug(std::vector<std::vector<double>>& mTermsChain1,
    ...
    if (currentSite > _simParams->numSpinsDamped and currentSite <= testLength) {
        ...
        std::vector<double> originSite = {mTermsChain1[currentSite][0] * _simParams->PERMITTIVITY_IRON,
                                          mTermsChain1[currentSite][1] * _simParams->PERMITTIVITY_IRON,
                                          mTermsChain1[currentSite][2] * _simParams->PERMITTIVITY_IRON};
        if (_simFlags->debugFunc) { std::cout << "INTER Origin: [" << originSite[0] << ", " << originSite[1] << ", " << originSite[2] << "] | ";}

        if (_simFlags->debugFunc) { std::cout << "DB1.7 | "; }
        std::vector<double> influencingSite = {mTermsChain2[currentSite][0] * _simParams->PERMITTIVITY_IRON,
                                               mTermsChain2[currentSite][1] * _simParams->PERMITTIVITY_IRON,
                                               mTermsChain2[currentSite][2] * _simParams->PERMITTIVITY_IRON};
        if (_simFlags->debugFunc) { std::cout << "INTER Influe: [" << influencingSite[0] << ", " << influencingSite[1] << ", " << influencingSite[2] << "] | ";}
