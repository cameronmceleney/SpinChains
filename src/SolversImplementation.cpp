    ...
    for (int iteration = simParams->iterationStart; iteration <= simParams->iterationEnd; iteration++) {
        ...
        for (int site = 1; site <= GV.GetNumSpins(); site++) {
            ...
            */
            ...
            double myK1 = llg.MagneticMomentY(site, simStates->mx0[site], simStates->my0[site], simStates->mz0[site], hxK0, hyK0, hzK0);
            double mzK1 = llg.MagneticMomentZ(site, simStates->mx0[site], simStates->my0[site], simStates->mz0[site], hxK0, hyK0, hzK0);
        ...
        for (int layer = 0; layer < simParams->numLayers; layer++) {
            ...
            for (int site = 1; site <= simStates->layerTotalSpins[layer]; site++) {
                ...
                if (simFlags->hasDipolar) {
                    ...
                    if (simFlags->debugFunc) {std::cout << "\n\niteration: " << iteration << " | layer: " << layer << " | site: " << site << std::endl;}
                    std::vector<double> dipoleTerms = dipolarField.DipolarInteractionInterlayer(simStates->m0Nest[layer1], simStates->m0Nest[layer2], site,
                                                                                                layer1, layer2);
        ...
        for (int layer = 0; layer < simParams->numLayers; layer++) {
            ...
            for (int site = 1; site <= simStates->layerTotalSpins[layer]; site++) {
                ...
                if (simFlags->hasDipolar) {
                    ...
                    if (simFlags->debugFunc) { simFlags->debugFunc = false; debugCounter++; }
                    std::vector<double> dipoleTerms = dipolarField.DipolarInteractionInterlayer(simStates->m1Nest[layer1], simStates->m1Nest[layer2], site,
                                                                                                layer1, layer2);
