...
void BiasFields::calculateOneDimension( const int &currentLayer, const double &currentTime,
    ...
    for ( int i = 1; i <= _simParams->systemTotalSpins; i++ ) {
        ...
        exchangeXOut[i] = tempResultsContainer[0];
        exchangeYOut[i] = tempResultsContainer[1];
        exchangeZOut[i] = tempResultsContainer[2];
    }
}
        ...
        tbb::parallel_for(tbb::blocked_range<int>(1, _simParams->systemTotalSpins),
                          [&]( const tbb::blocked_range<int> tbbRange ) {
                              for ( int i = tbbRange.begin(); i <= tbbRange.end(); i++ ) {
                                  ...
                                  std::array<double, 3> tempResultsLocal = _calculateBiasField1D(i, currentLayer,
                                                                                                 currentTime,
                                                                                                 shouldUseTBB);
