...
void NMConfiguration::Configure() {
    ...
    if (useMultilayer) {
        _generateMultilayerAbsorbingRegions(numSpinsDamped, gilbertDamping, gilbertABCInner,
                                            gilbertABCOuter);
    } else {
        _generateAbsorbingRegions(numSpinsInChain, numSpinsDamped, gilbertDamping, gilbertABCInner, gilbertABCOuter);}
    _setupDrivingRegion(numSpinsInChain, numSpinsDamped, drivingRegionWidth);
    _generateExchangeVector(numSpinsDamped, numberOfSpinPairs, exchangeEnergyMin, exchangeEnergyMax);

    if (useMultilayer) {
         m0Nest = InitialiseNestedVectors(totalLayers, _mxInit, _myInit, _mzInit);
         m1Nest = InitialiseNestedVectors(totalLayers, _mxInit, _myInit, zeroValue);
         m2Nest = InitialiseNestedVectors(totalLayers, _mxInit, _myInit, zeroValue);
...
void NMConfiguration::_generateAbsorbingRegions(int numSpinsInChain, int numSpinsAbsorbingRegion, double gilbertSpinChain,
    ...
    gilbertVector.insert(gilbertVector.end(), dampingRegionLHS.begin(), dampingRegionLHS.end());
    gilbertVector.insert(gilbertVector.end(), gilbertChain.begin(), gilbertChain.end());
    gilbertVector.insert(gilbertVector.end(), dampingRegionRHS.begin(), dampingRegionRHS.end());
    gilbertVector.push_back(0);
}

void NMConfiguration::_setupDrivingRegion(int numSpinsInChain, int numSpinsAbsorbingRegion, int drivingRegionWidth) {

    if (centralDrive) {
        drivingRegionLhs = (numSpinsInChain / 2) + numSpinsAbsorbingRegion - (drivingRegionWidth / 2);
        drivingRegionRhs = (numSpinsInChain / 2) + numSpinsAbsorbingRegion + (drivingRegionWidth / 2);
        return;
    }

    if (dualDrive) {throw std::runtime_error(std::string("dualDrive has been selected, but this feature is not yet implemented"));}

    if (lhsDrive) {
        // The +1/-1 offset excludes the zeroth spin while retaining the correct driving width
        drivingRegionLhs = numSpinsAbsorbingRegion + 1;
        drivingRegionRhs = drivingRegionLhs + drivingRegionWidth - 1;
        return;
    }

    if (!rhsDrive) {
        // The +1 is to correct the offset of adding a zeroth spin
        drivingRegionRhs = systemTotalSpins - numSpinsAbsorbingRegion - 1;
        drivingRegionLhs = drivingRegionRhs - drivingRegionWidth + 1;
        return;
    }
...
void NMConfiguration::_generateExchangeVector(int numSpinsAbsorbingRegion, int numSpinPairs, double exchangeMin, double exchangeMax) {
    ...
    if (numSpinsAbsorbingRegion > 0) {
        ...
        exchangeVec = SpinChainExchange.generate_array();
        ...
        exchangeVec.insert(exchangeVec.begin(), dampingRegionLeftExchange.begin(), dampingRegionLeftExchange.end());
        exchangeVec.insert(exchangeVec.end(), dampingRegionRightExchange.begin(), dampingRegionRightExchange.end());
    } else {
...
void NMConfiguration::_setupInitMagneticMoments(double mxInit, double myInit, double mzInit) {
    ...
    std::vector<double> mxInitCond(systemTotalSpins, mxInit), myInitCond(systemTotalSpins, myInit), mzInitCond(systemTotalSpins, mzInit);
...
void NMConfiguration::_testShockwaveInitConditions() {
    ...
    if (hasShockwave) {
        shockwaveStepsize = (shockwaveMax - shockwaveInitialStrength) / shockwaveGradientTime;
    } else {
        // Ensures, on the output file, all parameter read as zero; reduces confusion when no shockwave is applied.
        iterStartShock = 0.0;
        shockwaveScaling = 0.0;
        shockwaveGradientTime = 0.0;
        shockwaveInitialStrength = 0.0;
        shockwaveMax = 0.0;
        shockwaveStepsize = 0.0;
    }
}
...
void NMConfiguration::_generateMultilayerAbsorbingRegions(int numSpinsAbsorbingRegion, double gilbertSpinChain,
    ...
    for (int& layer: layerSpinsInChain) {
        ...
        // Combine all damped regions to form vector which describes the entire spinchain.
        gilbertVectorMulti[layer].insert(gilbertVectorMulti[layer].end(), dampingRegionLHS.begin(), dampingRegionLHS.end());
        gilbertVectorMulti[layer].insert(gilbertVectorMulti[layer].end(), gilbertChain.begin(), gilbertChain.end());
        gilbertVectorMulti[layer].insert(gilbertVectorMulti[layer].end(), dampingRegionRHS.begin(), dampingRegionRHS.end());
        gilbertVectorMulti[layer].push_back(0);
