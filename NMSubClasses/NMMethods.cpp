...
void NMMethods::_testShockwaveConditions(double iteration) {
    ...
    if (shouldDriveCease) {
        ...
        if (isShockwaveOn and not isShockwaveAtMax) 
            std::cout << "Shock not at maximum when cut-off" << std::endl;

        if (iteration >= iterationEnd * iterEndShock) {
    ...
    if (hasShockwave and not isShockwaveOn)
    {
        if (iteration >= iterationEnd * iterStartShock)
        {
            // Shockwave begins once simulation is a certain % complete
            isShockwaveOn = true;
            dynamicBiasField = shockwaveInitialStrength;
        }
    ...
    if (isShockwaveOn and not isShockwaveAtMax)
    {
        dynamicBiasField += shockwaveStepsize;

        if (dynamicBiasField >= shockwaveMax)
        {
            dynamicBiasField = shockwaveMax;
            isShockwaveAtMax = true;
...
void NMMethods::SolveRK2Classic() {
    ...
    if (isFm) {
        NMData -> InformUserOfCodeType("RK2 Midpoint (FM)");
        NMData -> CreateFileHeader(mxRK2File, "RK2 Midpoint (FM)");
        //NMData -> CreateFileHeader(myRK2File, "RK2 Midpoint (FM)");
        //NMData -> CreateFileHeader(mzRK2File, "RK2 Midpoint (FM)");

    } else if (!isFm) {
        NMData -> InformUserOfCodeType("RK2 Midpoint (AFM)");
        NMData -> CreateFileHeader(mxRK2File, "RK2 Midpoint (AFM)");
    }

    if (GV.GetEmailWhenCompleted()) {
        NMData -> CreateMetadata();
    }
    ...
    for (int iteration = iterationStart; iteration <= iterationEnd; iteration++) {
        ...
        if (iterationEnd >= 100 && iteration % (iterationEnd / 100) == 0)
        ...
        // EASY FIND
        if (useDipolar)
            Dipolar -> DipolarInteraction1D(mx0, dipoleX);
        if (useDemagIntense) {
            Demag -> DemagnetisationFieldIntense(demagX, demagY, demagZ, mx0, my0, mz0);
        } else if (useDemagFft) {
            std::string rkStageName = "2-1";
            Demag -> DemagField1DReal(demagX, demagY, demagZ, mx0, my0, mz0, iteration, rkStageName);
        ...
        for (int site = 1; site <= GV.GetNumSpins(); site++) {
            ...
            */
            ...
            double hxK0 = EffField -> EffectiveFieldX(site, 0, mx0[spinLHS], mx0[site], mx0[spinRHS], dipoleX[site], demagX[site], t0);
            double hyK0 = EffField -> EffectiveFieldY(site, 0, my0[spinLHS], my0[site], my0[spinRHS], dipoleY[site], demagY[site]);
            double hzK0 = EffField -> EffectiveFieldZ(site, 0, mz0[spinLHS], mz0[site], mz0[spinRHS], dipoleZ[site], demagZ[site]);

            // RK2 K-value calculations for the magnetic moment, coded as symbol 'm', components of the target site
            double mxK1 = LLG -> MagneticMomentX(site, mx0[site], my0[site], mz0[site], hxK0, hyK0, hzK0);
            double myK1 = LLG -> MagneticMomentY(site, mx0[site], my0[site], mz0[site], hxK0, hyK0, hzK0);
            double mzK1 = LLG -> MagneticMomentZ(site, mx0[site], my0[site], mz0[site], hxK0, hyK0, hzK0);

            // Find (m0 + k1/2) for each site, which is used in the next stage.
            mx1[site] = mx0[site] + stepsizeHalf * mxK1;
            my1[site] = my0[site] + stepsizeHalf * myK1;
            mz1[site] = mz0[site] + stepsizeHalf * mzK1;
        }
        ...
        for (int site = 1; site <= GV.GetNumSpins(); site++) {
            ...
            */
            ...
            mx2[site] = mx0[site] + stepsize * mxK2;
            my2[site] = my0[site] + stepsize * myK2;
            mz2[site] = mz0[site] + stepsize * mzK2;

            if (shouldTrackMValues) {
                double mIterationNorm = sqrt(pow(mx2[site], 2) + pow(my2[site], 2) + pow(mz2[site], 2));
                if ((largestMNorm) > (1.0 - mIterationNorm)) { largestMNorm = (1.0 - mIterationNorm); }
         ...
         */
        mx0.clear(); my0.clear(); mz0.clear();
        mx1.clear(); my1.clear(); mz1.clear();

        NMData -> SaveDataToFile(mxRK2File, mx2, iteration);
        //NMData -> SaveDataToFile(myRK2File, my2, iteration);
        //NMData -> SaveDataToFile(mzRK2File, mz2, iteration);

        //Sets the final value of the current iteration of the loop to be the starting value of the next loop.
        mx0 = mx2; my0 = my2; mz0 = mz2;

        if (iteration == forceStopAtIteration)
            exit(0);
    ...
    if (GV.GetEmailWhenCompleted()) {
        NMData -> CreateMetadata(true);
    }

    if (shouldTrackMValues)
        std::cout << "\nMax norm. value of M is: " << largestMNorm << std::endl;
...
void NMMethods::SolveRK2() {
    ...
    if (isFm) {
        NMData -> InformUserOfCodeType("RK2 Midpoint (FM)");
        NMData -> CreateFileHeader(mxRK2File, "RK2 Midpoint (FM)", false, 0);
        NMData -> CreateFileHeader(mxRK2File1, "RK2 Midpoint (FM)", false, 1);
    } else if (!isFm) {
        NMData -> InformUserOfCodeType("RK2 Midpoint (AFM)");
        NMData -> CreateFileHeader(mxRK2File, "RK2 Midpoint (AFM)");
        NMData -> CreateFileHeader(mxRK2File1, "RK2 Midpoint (AFM)");
    }
    ...
    for (int iteration = iterationStart; iteration <= iterationEnd; iteration++) {
        ...
        for (int layer = 0; layer < totalLayers; layer++) {
            ...
            for (int site = 1; site <= layerTotalSpins[layer]; site++) {
                // Exclude the 0th and last spins as they will always be zero-valued (end, pinned, bound spins)
                ...
                if (useDipolar) {
                    ...
                    std::vector<double> dipoleTerms = Dipolar -> DipolarInteractionInterlayer(m0Nest[layer1], m0Nest[layer2], site,
                                                                                   layer1, layer2);
                ...
                // Find (m0 + k1/2) for each site, which is used in the next stage.
                m1Nest[layer][site][0] = mxMID + stepsizeHalf * mxK1;
                m1Nest[layer][site][1] = myMID + stepsizeHalf * myK1;
                m1Nest[layer][site][2] = mzMID + stepsizeHalf * mzK1;
            }
        }
        ...
        for (int layer = 0; layer < totalLayers; layer++) {
            ...
            for (int site = 1; site <= layerTotalSpins[layer]; site++) {
                ...
                if (useDipolar) {
                    ...
                    if (debugFunc) { debugFunc = false; debugCounter++; }
                    std::vector<double> dipoleTerms = Dipolar -> DipolarInteractionInterlayer(m1Nest[layer1], m1Nest[layer2], site,
                                                                                   layer1, layer2);
                    if (debugCounter > 0) { debugFunc = true; }
                ...
                m2Nest[layer][site][0] = m0Nest[layer][site][0] + stepsize * mxK2;
                m2Nest[layer][site][1] = m0Nest[layer][site][1] + stepsize * myK2;
                m2Nest[layer][site][2] = m0Nest[layer][site][2] + stepsize * mzK2;

                if (shouldTrackMValues) {
                    double mIterationNorm = sqrt(
                            pow(m2Nest[layer][site][0], 2) + pow(m2Nest[layer][site][1], 2) + pow(m2Nest[layer][site][2], 2));
                    if ((largestMNormMulti[layer]) > (1.0 - mIterationNorm)) { largestMNormMulti[layer] = (1.0 - mIterationNorm); }
                }
            }
        }
        ...
        NMData -> SaveDataToFileMultilayer(mxRK2File, m2Nest[0], iteration, 0);
        NMData -> SaveDataToFileMultilayer(mxRK2File1, m2Nest[1], iteration, 1);
    ...
    if (shouldTrackMValues) {
        ...
        for (int i = 0; i < largestMNormMulti.size(); i++) {
            std::cout << "Layer " << i << ": " << largestMNormMulti[i] << " | ";
        }
    }
...
void NMMethods::runMethod(const std::string& methodToUse) {
